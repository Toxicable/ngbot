...
const msgContainsReplies = {
  ...
  'route reuse': ` http://stackoverflow.com/questions/41280471/how-to-implement-routereusestrategy-shoulddetach-for-specific-routes-in-angular/41515648#41515648`
};
...
function getReply(text) {
  ...
  if (text.startsWith(botKeyWord)) {
    ...
    let reply = '';
    Object.keys(msgContainsReplies)
      .forEach(key => {
        let containsKey = key.toLowerCase()
          .split()
          .every(part => text.includes(part))

        if (containsKey) {
          reply = msgContainsReplies[key];
        }
        //.map(part => text.includes(part))
    ...
    if (!reply) {
      ...
      return otherReplies.noStoredReply;
    }
    else {
      return reply;
    }
